...
if dM > 0
   ...
   a = Grad1_Li(PF(end-5:end,:),1,1);  %%Calculate the gradient of the last ten values of the sound velocity profile
...
for i = 1 : m-1
    ...
    [t1, x1, z1,L1,cos_alfa0,cv] = SingleLayerTracing_Li(cos_alfa0,c0,a,+inf,+inf,z);% ray tracing of a single layer
  ...
  if min(V) < inf       % Determine whether the last layer of sound trace calculation is needed
      [t1, x1, z1,L1,cos_alfa1,c1] = SingleLayerTracing_Li(cos_alfa00,c0,a,V(1),V(2),V(3)); % ray tracing of last layer
      LL(i) = L1; ts(i) = t1; xx(i) = x1; zz(i) = z1;
     break;
  end
end
