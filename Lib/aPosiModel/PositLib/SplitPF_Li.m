...
[vel StartIndex] = DepthVelocity_Li(profile,StartDepth,'ceil'); % fetch a start point
[vel EndIndex]   = DepthVelocity_Li(profile,EndDepth,'flor');   % fetch a end point
SubProfile       = profile(StartIndex:EndIndex,:);           % fetch the sub profile

h0 = SubProfile(1,1);                                        % the starting depth
dh = StartDepth - h0;
if dh > 0
   a = Grad1_Li(SubProfile(1:2,:),1,1);                           % caculate the sound velocity gradient
...
dh = EndDepth - h0;
if dh <= 0
   a = Grad1_Li(SubProfile(end-1:end,:),1,1);                      % caculate the sound velocity gradient
   SubProfile(end,2) = SubProfile(end,2) + a * dh;              % interpolation for the
   SubProfile(end,1) = EndDepth;
else
   a = Grad1_Li(SubProfile(end-100:end,:),1,1);                      % caculate the sound velocity gradient
